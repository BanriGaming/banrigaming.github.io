...
function generateDataCards(data) {
    ...
    data.forEach(planet => {
        ...
        Object.keys(planet).forEach(key => {
            if (planet[key] === 'X' && key !== 'Pal' && key !== 'Location' && key !== 'Type') {
        ...
        card.innerHTML = `
            <div class="card-header">
                ...
                <h6 class="card-subtitle mb-2 text-muted">${planet['Pal']}</h6>
        ...
        resources.splice(resources.indexOf('Pal'), 1); // Remove 'System' from resources
    ...
    const filteredData = data.filter(planet => {
        if (searchType === 'search-star-pal') {
            if (!selectedFilters.length) {
                return planet['Pal'].toLowerCase().includes(searchInput);
            } else {
                return (
                    planet['Pal'].toLowerCase().includes(searchInput) &&
