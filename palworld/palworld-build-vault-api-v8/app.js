...
(async () => {
  ...
  function enhanceSearchPickers(){
    $$('.smart-picker').forEach(w=>{
      ...
      input.addEventListener('focus',draw);input.addEventListener('input',draw);button.onclick=draw;
  ...
  function renderLibrary(){
    ...
    $$('[data-favorite]',els.libraryList).forEach(b=>b.onclick=e=>{e.stopPropagation();const pal=state.library.find(p=>p.id===b.dataset.favorite);if(p){pal.favorite=!pal.favorite;persist();renderLibrary()}});
  }
  function libraryCard(p){return `<article class="library-card" style="--element:${color(p.elements[0])}" data-library-card="${p.id}">${img(p,"library-card__image")}<div class="library-card__content"><div class="library-card__top"><div><div class="library-row__title">${esc(p.nickname||p.species)} ${p.elements[0]?`<span class="element-badge">${esc(p.elements[0])}</span>`:""}</div><div class="library-card__species">${esc(p.species)} · Lv. ${esc(p.level||"—")} · ${esc(p.condensation||0)}★</div></div><button class="favorite-button" data-favorite="${p.id}" type="button" aria-label="Favorite">${p.favorite?"★":"☆"}</button></div><div class="storage-chip">▣ ${esc(storageLabel(p))}</div><div class="library-card__skills">${p.activeSkills.filter(Boolean).slice(0,3).map(x=>`<span class="skill-chip">${esc(x)}</span>`).join("")||`<span class="skill-chip">No active skills</span>`}</div><div class="library-card__actions"><button class="button button--small" data-edit-library="${p.id}" type="button">Edit</button><button class="button button--small button--cyan" data-add-party="${p.id}" type="button">Add to Party</button></div></div></article>`}
  ...
  function openLibrary(id=null){editingLibraryId=id;const p=id?state.library.find(x=>x.id===id):emptyPal();$("#libraryDialogTitle").textContent=id?"Edit Library Pal":"Add Pal to Library";$("#deleteLibraryPalButton").hidden=!id;setForm(p);els.libraryDialog.showModal();if(p.species&&PalworldAPI.configured){ensurePalReference(p.species).then(()=>showReference(p.species))}$("#libSpecies").focus()}
  ...
  function readForm(){return normalizePal({id:editingLibraryId||uid(),species:$("#libSpecies").value.trim(),nickname:$("#libNickname").value.trim(),role:$("#libRole").value.trim(),elements:[$("#libElement1").value,$("#libElement2").value],level:$("#libLevel").value,condensation:$("#libCondensation").value,storageType:$("#libStorageType").value,storageName:$("#libStorageName").value.trim(),storagePage:$("#libStoragePage").value,activeSkills:[0,1,2].map(i=>$("#libActive"+i).value.trim()),passives:[0,1,2,3].map(i=>$("#libPassive"+i).value.trim()),ivs:{hp:$("#libHp").value,attack:$("#libAttack").value,defense:$("#libDefense").value},workSuitability:Object.fromEntries(SUITABILITIES.map((x,i)=>[x,Number($("#suit"+i).value)||0])),notes:$("#libNotes").value.trim(),createdAt:editingLibraryId?(state.library.find(x=>x.id===editingLibraryId)?.createdAt||Date.now()):Date.now(),updatedAt:Date.now()})}
